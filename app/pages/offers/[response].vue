<template>
  <div v-if="data">
    ...
    <h2>Services</h2>
    <ul v-if="data.offerServices && data.offerServices.length">
      <li
        v-for="(serviceItem, sIdx) in data.offerServices"
        :key="'service-' + sIdx"
        class="mb-4">
        ...
        <div
          v-if="serviceItem.offerProducts && serviceItem.offerProducts.length"
          class="ml-4 mt-2">
          <h3>Products for this service:</h3>
          <ul>
            <li
              v-for="(productItem, pIdx) in serviceItem.offerProducts"
              :key="'product-' + sIdx + '-' + pIdx">
...
import {
  type OfferResponseSelect,
  type OfferResponseInsert,
...
const {data, error, status} = await useLazyFetch<OfferResponseSelect>(
  `/api/offers/response/${route.params.response}`,
  {
    method: 'get',
  }
);
...
const submit = async (payload: FormSubmitEvent<OfferResponseInsert>) => {
  try {
    await $fetch(`/api/offers/response/${route.params.response}`, {
      method: 'POST',
      body: payload.data,
    });
    ...
    toast.add({
      title: 'Success',
      description: 'The updated offer was sent to us!',
      color: 'success',
    });
  } catch (err) {
