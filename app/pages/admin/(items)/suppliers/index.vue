...
const {data, status, refresh} = await useFetch<SupplierSelect[]>(
  '/api/suppliers',
  {
    key: 'suppliers',
    method: 'get',
...
const getRowSuppliers = (row: Row<SupplierSelect>) => {
  return [
    ...
    },
    {
      ...
      icon: 'i-lucide-phone',
      onSelect: () => {
        window.location.href = `tel:${row.original.phoneNumber}`;
      },
    },
    {
      label: 'Send SMS',
      ...
      onSelect: () => {
        const message =
          "Happy New Year! How about a New Year's discount to celebrate? 😏";
